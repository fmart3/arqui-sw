...
async function atenderPaciente(contenido) {
  ...
  try {
    ...
    if (respuestaDatosPaciente.contenido[0].estado_atencion >= 2) {
      return { status: 0, contenido: 'El paciente ya está en estado alta.' };
    }
...
async function ingresarDiagnostico(contenido) {
  ...
  try {
    const query0 = `SELECT * FROM diagnostico WHERE id = '${diagnostico}'`;
    const respuesta0 = await client(servicioDataB, { accion: 'select', contenido: query0 });
    if (respuesta0.status === 0 || respuesta0.contenido[0] == null) {
      return { status: 0, contenido: 'Diagnostico no disponible en la base de datos.' };
    }
    const query = `
        UPDATE atencion
        SET id_diagnostico = '${diagnostico}'
        WHERE id = ${idAtencion};
      `;
    const respuesta = await client(servicioDataB, { accion: 'update', contenido: query });
    return respuesta.status === 1
...
async function consultarDiagnosticos() {
  try {
    const query = 'SELECT * FROM diagnostico'
    const respuesta = await client(servicioDataB, { accion: 'select', contenido: query });
    if (respuesta.status === 0) {
      return { status: 0, contenido: 'Error al obtener los diagnosticos.' };
    }
...
async function consultarCentros() {
  try {
    const query = 'SELECT * FROM centro_salud'
    const respuesta = await client(servicioDataB, { accion: 'select', contenido: query });
    if (respuesta.status === 0) {
      return { status: 0, contenido: 'Error al obtener los centro de salud.' };
    }
